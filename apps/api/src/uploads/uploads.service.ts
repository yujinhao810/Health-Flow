...
export class UploadsService {
  ...
  async create(user: AuthUser, file: Express.Multer.File | undefined, purpose: UploadedFilePurpose) {
    ...
    const extractedText = this.extraction.extract(file.buffer, file.mimetype, originalName);

    if (purpose === 'knowledge_source' && !extractedText) {
      throw new BadRequestException('这个文件没有提取到可用于知识库检索的文本；请上传文本型 PDF、docx、txt、md、csv 或 json 文件');
    }
    ...
    if (purpose === 'knowledge_source') {
      try {
        const document = await this.documents.ingestUploadedFile(user, uploaded, extractedText ?? '');
  ...
  async list(user: AuthUser, purpose?: UploadedFilePurpose) {
    if (purpose && !PURPOSES.includes(purpose)) throw new BadRequestException('不支持的上传用途');
  ...
  toPublicAttachment(file: UploadedFile): ChatAttachment {
    ...
    return {
      ...
      status: file.status,
      documentId: typeof metadata.knowledgeDocumentId === 'string' ? metadata.knowledgeDocumentId : undefined,
      chunkCount: typeof metadata.chunkCount === 'number' ? metadata.chunkCount : undefined,
      hasExtractedText: typeof metadata.hasExtractedText === 'boolean' ? metadata.hasExtractedText : undefined,
      contentUrl: `/uploads/${file.id}/content`,
      createdAt: file.createdAt.toISOString(),
    };
  }

  buildAttachmentContext(files: UploadedFile[], options?: { visionEnabled?: boolean }) {
  ...
  async buildUserMessageContent(content: string, files: UploadedFile[], options?: { visionEnabled?: boolean }): Promise<string | LlmContentBlock[]> {
    const imageFiles = options?.visionEnabled ? files.filter((file) => getSupportedImageMimeType(file)).slice(0, 4) : [];
  ...
  private assertAllowed(file: Express.Multer.File) {
    ...
    if (allowed.size && !allowed.has(file.mimetype) && !isAllowedUploadExtension(file.originalname)) {
      throw new BadRequestException(`不支持的文件类型：${file.mimetype || file.originalname}`);
    }
  }
...
function isAllowedUploadExtension(filename: string) {
  return /\.(png|jpe?g|webp|gif|bmp|pdf|docx?|txt|md|markdown|json|csv)$/i.test(filename);
}
...
function normalizeOriginalName(value: string) {
  const cleaned = value.replace(/[\\/]/g, '_').replace(/[\u0000-\u001f]/g, '').trim() || 'upload';
  const decoded = decodeLatin1Utf8(cleaned);
  return readabilityScore(decoded) > readabilityScore(cleaned) + 0.15 ? decoded : cleaned;
}
